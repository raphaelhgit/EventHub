...
async function createUser(dto) {
  ...
  `).run(id, dto.email, passwordHash, dto.name, dto.role ?? UserRole.USER, createdAt, createdAt);
    return { id, email: dto.email, name: dto.name, role: dto.role ?? UserRole.USER, createdAt: new Date(createdAt), updatedAt: new Date(createdAt) };
}
async function findUserByEmail(email) {
    const row = sqliteDb.prepare('SELECT * FROM users WHERE email = ?').get(email);
    if (!row)
        return undefined;
    return { id: row.id, email: row.email, passwordHash: row.password_hash, name: row.name, role: row.role, createdAt: new Date(row.created_at), updatedAt: new Date(row.updated_at) };
}
...
function createEvent(dto, organizerId) {
    const id = uuidv4();
    const createdAt = now();
    sqliteDb.prepare(`
...
function findEventsByOrganizer(organizerId) {
    const rows = sqliteDb.prepare('SELECT * FROM events WHERE organizer_id = ? ORDER BY date ASC').all(organizerId);
    return rows.map(mapRowToEvent);
}
function updateEvent(id, dto) {
    ...
    if (dto.totalPlaces !== undefined) {
        ...
        params.push(Math.max(0, dto.totalPlaces - sold));
    }
    if (updates.length === 0)
        return event;
...
function deleteEvent(id) {
    ...
    if (!event)
        return false;
    const sold = sqliteDb.prepare('SELECT COUNT(*) as count FROM tickets WHERE event_id = ? AND status != ?').get(id, TicketStatus.CANCELLED);
...
function findTicketById(id) {
    const row = sqliteDb.prepare('SELECT * FROM tickets WHERE id = ?').get(id);
    if (!row)
        return undefined;
    return mapRowToTicket(row);
}
function findTicketsByUser(userId) {
    const rows = sqliteDb.prepare('SELECT * FROM tickets WHERE user_id = ? ORDER BY purchase_date DESC').all(userId);
    return rows.map(mapRowToTicket);
}
function findTicketsByEvent(eventId) {
    const rows = sqliteDb.prepare('SELECT * FROM tickets WHERE event_id = ?').all(eventId);
    return rows.map(mapRowToTicket);
}
...
function getTicketStatsByOrganizer(organizerId) {
    const rows = sqliteDb.prepare(`SELECT t.* FROM tickets t JOIN events e ON t.event_id = e.id WHERE e.organizer_id = ?`).all(organizerId);
    let totalRevenue = 0;
...
function mapRowToEvent(row) {
    return { id: row.id, title: row.title, description: row.description, date: row.date, time: row.time, location: row.location, city: row.city, price: row.price, totalPlaces: row.total_places, availablePlaces: row.available_places, category: row.category, image: row.image ?? undefined, organizerId: row.organizer_id, createdAt: new Date(row.created_at), updatedAt: new Date(row.updated_at) };
}
function mapRowToTicket(row) {
    return { id: row.id, qrCode: row.qr_code, eventId: row.event_id, userId: row.user_id, status: row.status, purchaseDate: new Date(row.purchase_date), usedAt: row.used_at ? new Date(row.used_at) : undefined, cancelledAt: row.cancelled_at ? new Date(row.cancelled_at) : undefined };
}
export const db = {
